...
export class SlumpgeneratorLocationComponent {
  ...
  getContent(pairingMode: string, index: number): string {
    const words = Object.values(BildbegreppWords);
    switch (pairingMode) {
      case 'ord': // WORD mode
        return words[index % words.length]; // Cycle through words
      case 'bild': // ILLUSTRATION mode
        return `/assets/images/${words[index % words.length]}.png`; // Assume images are stored with this path
      case 'ritade tecken': // RITADE_TECKEN mode
        return `/assets/drawings/${words[index % words.length]}.png`; // Path to drawings
      case 'tecken som stöd': // TAKK mode
        return `/assets/videos/${words[index % words.length]}.mp4`; // Path to videos
      default:
        return '';
    }
  }
